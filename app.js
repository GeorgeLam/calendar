...
delBtnEnable = function () {
    ...
    for (let i = 0; i < editBtn.length; i++){
        editBtn[i].addEventListener("click", () => {
            ...
            editBtn[i].parentElement.previousElementSibling.innerHTML = `<input type="text" class="newText" value="${editBtn[i].parentElement.previousElementSibling.innerText}"><button class="saveEdit">Save Edit</button>`
...
let saveEditCheck = function() {
    ...
    saveEdit[i].addEventListener("click", () => {
        if (i >= saveEdit.length) { i = saveEdit.length - 1; console.log("Correcting") }
        console.log("You just clicked save on button " + i)
        console.log(document.querySelectorAll("li").length);

        let savedNum = i
        let numb = saveEdit[i].parentElement.parentElement.firstElementChild.id[saveEdit[i].parentElement.parentElement.firstElementChild.id.length-1]
        
        //editBtn[savedNum].parentElement.previousElementSibling.innerHTML = newTexts[savedNum].value;
        document.querySelector(`#list-text${numb}`).innerHTML = newTexts[savedNum].value;
    })
}
}
